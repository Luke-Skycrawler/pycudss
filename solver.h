...
struct SolverBase {
    ...
    SolverBase(const Veci &outers, const Veci &indices, const Vec &values)
        : outers(outers), indices(indices), values(values), n(outers.rows()), nnz(indices.rows()) {}
    SolverBase(const Eigen::SparseMatrix<double> &A): n(A.outerSize()), nnz(A.nonZeros()) {
        ...
        outers.resize(A.outerSize());
        indices.resize(A.nonZeros());
        values.resize(A.nonZeros());


        memcpy(outers.data(), outer_ptr, A.outerSize() * sizeof(int));
...
struct CUSolver: SolverBase {
    ...
    ~CUSolver() {
        ...
        cudssDestroyData(solver_data);
        cudssDestroyConfig(solver_config);
        cudssDestroyHandle(handle);
    }
